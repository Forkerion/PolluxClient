...
package rardoger.polluxclient.utils.player;
...
public class Chat {
    ...
    private static void sendMsg(int id, Module module, String msg, Formatting color) {
        ...
        if (module != null) {
            ((IChatHud) mc.inGameHud.getChatHud()).add(new LiteralText(String.format("%s[%sPollux%s] %s[%s] %s%s", Formatting.GRAY, Formatting.BLUE,Formatting.GRAY, Formatting.AQUA, module.title, color, msg)), id);
        } else {
            ((IChatHud) mc.inGameHud.getChatHud()).add(new LiteralText(String.format("%s[%sPollux%s] %s%s", Formatting.GRAY, Formatting.BLUE, Formatting.GRAY, color, msg)), id);
        }
    }
