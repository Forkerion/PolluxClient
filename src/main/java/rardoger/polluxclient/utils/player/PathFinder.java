...
import rardoger.polluxclient.events.entity.player.PlayerMoveEvent;
import rardoger.polluxclient.mixininterface.IKeyBinding;
import rardoger.polluxclient.modules.ModuleManager;
import rardoger.polluxclient.modules.movement.Step;
...
public class PathFinder {
    ...
    public void initiate(Entity entity) {
        ...
        if (target != null)
            currentPathBlock = getNextPathBlock();
        PolluxClient.EVENT_BUS.subscribe(moveEventListener);
    }
    ...
    public void disable() {
        ...
        if (mc.options.keyForward.isPressed())
            ((IKeyBinding) mc.options.keyForward).setPressed(false);
        PolluxClient.EVENT_BUS.unsubscribe(moveEventListener);
    }
